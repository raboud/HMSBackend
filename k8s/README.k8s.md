...
1. Open a PowerShell command line at the `k8s` directory of your local HMSBackend repository.
...
>```
>az acr credential show -n hmsregistry
>```
...
+ `buildBits`: means that the source code of HMSBackend will be built. If you have built your code (and have all projects published in `obj/Docker/publish`) do not pass this parameter. Default value is `false`
...
+ `dockerOrg`: Name of the organization in the registry where the images are (or will be pushed). Default value is `hms` (which has images provided by Microsoft)
